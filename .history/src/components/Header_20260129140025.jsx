  ...
  return (
    <header className="absolute w-full z-50 transition-all duration-300">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16 md:h-20">
        ...
        >
          <span className="px-0 text-2xl font-bold bg-gradient-to-r from-gray-300 to-gray-100 bg-clip-text text-transparent">
            Abhi Chaddha
          </span>
        ...
        <nav className="lg:flex hidden space-x-8">
          {["Home", "About", "Projects", "Contact"].map((item) => (
            ...
            variant = "bordered"
              ...
              className="relative text-gray-800 dark:text-gray-200 hover:light-red-600 dark:hover:text-violet-400 font-medium transition-colors duration-300 group"
              href="#"
            >
              {item}
          ...
          <motion.button
            onClick={openContactForm} // <-- just call the prop
            initial={{ opacity: 0, scale: 0.8 }}
            animate={{ opacity: 1, scale: 1 }}
            transition={{ delay: 1.6, duration: 0.8, type: "spring", stiffness: 100, damping: 15 }}
            className="ml-4 px-4 py-0.5 rounded-xl bg-gradient-to-r from-gray-200 to-gray-300 text-red-700 font-bold hover:from-red-700 hover:to-red-700 hover:text-white transition-all duration-400"
          >
            Hire Me
          </motion.button>
      </nav>
      ...
      {isOpen && (
        <div className="md:hidden bg-white dark:bg-gray-900 shadow-lg px-4 py-5 space-y-5">
          <nav className="flex flex-col space-y-3">
            {["Home", "About", "Projects", "Contact"].map((item) => (
              <a key={item} onClick={toggleMenu} className="text-gray-300 font-medium py-2" href="#">
                {item}
              </a>
            ))}
          </nav>
